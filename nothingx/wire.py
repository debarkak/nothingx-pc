...
def crc16(data: bytes) -> bytes:
    val = 0xFFFF
    for b in data:
        val = _CRC_TABLE[(val ^ b) & 0xFF] ^ (val >> 8)
    return bytes([val & 0xFF, (val >> 8) & 0xFF])


def build_packet(cmd: int, direction: int, payload: list, op_id: int = 1) -> bytes:
    length = len(payload)
    base = bytes([0x55, 0x60, 0x01, cmd, direction, length, 0x00, op_id] + payload)
